...
function Home() {
  ...
  const handleSeatConfirm = async (selectedSeats, movieId) => {
    ...
    if (paymentConfirmed) {
      try {
        await axios.post(
          'https://movie-ticket-booking-backend-60e5.onrender.com/bookings',
        ...
        fetchMovies();
        navigate('/my-bookings'); // Only redirect to my-bookings
      } catch (error) {
  ...
  return (
    <Container sx={{ mt: 4, pb: 4, maxWidth: { xs: '100%', sm: 'md', md: 'lg' } }}>
      <Typography
        variant="h3"
        gutterBottom
        color="white"
        sx={{ textShadow: '2px 2px 4px rgba(0,0,0,0.5)', fontSize: { xs: '1.5rem', sm: '2rem', md: '3rem' } }}
      >
        Explore Movies
      </Typography>
      <TextField
      ...
      >
        {categories.map(cat => (
          <Tab
            ...
            label={cat}
            value={cat}
            sx={{ color: '#fff', '&.Mui-selected': { color: '#ff5722' }, fontSize: { xs: '0.8rem', sm: '1rem' } }}
          />
        ))}
      </Tabs>
        ...
        {filteredMovies.map((movie) => {
          ...
          return (
            <Grid item xs={12} sm={6} md={4} lg={3} key={movie.id}>
              <Card sx={{ bgcolor: '#fff', borderRadius: 2, transition: 'transform 0.2s', '&:hover': { transform: 'scale(1.03)' }, height: '100%', display: 'flex', flexDirection: 'column' }}>
                <img
                  src={posterUrl}
                  alt={movie.title}
                  style={{
                ...
                <CardContent sx={{ flexGrow: 1 }}>
                  ...
                  <Typography color="textSecondary" sx={{ fontSize: { xs: '0.8rem', sm: '0.9rem' } }}>Genre: {movie.genre}</Typography>
                  <Typography sx={{ fontSize: { xs: '0.8rem', sm: '0.9rem' } }}>Available: {remainingTickets}/{movie.totalTickets}</Typography>
                  <TextField
                    label="Tickets"
                    type="number"
